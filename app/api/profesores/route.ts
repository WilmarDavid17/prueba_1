...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("[v0] GET /api/profesores called with rfid:", rfid)

    let result
    if (rfid) {
      result = await query("SELECT * FROM profesores WHERE rfid = $1", [rfid])
    } else {
      result = await query("SELECT * FROM profesores ORDER BY id DESC")
    }

    const rows = (result as any).rows || result
    return NextResponse.json(rfid ? rows[0] || null : Array.isArray(rows) ? rows : [])
  } catch (error: any) {
    console.error("[v0] GET /api/profesores Database error:", error.message, error.code)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] POST /api/profesores body:", body)
    ...
    console.error("[v0] POST /api/profesores error:", error.message, error.code)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log("[v0] PUT /api/profesores id:", id, "body:", body)

    if (!id) return NextResponse.json({ error: "ID es requerido" }, { status: 400 })

    if (nombre && rfid) {
      await query("UPDATE profesores SET nombre = $1, rfid = $2 WHERE id = $3", [nombre, rfid, id])
    } else if (nombre) {
      await query("UPDATE profesores SET nombre = $1 WHERE id = $2", [nombre, id])
    } else if (rfid) {
      await query("UPDATE profesores SET rfid = $1 WHERE id = $2", [rfid, id])
    } else {
    ...
    console.error("[v0] PUT /api/profesores error:", error.message, error.code)
...
export async function DELETE(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url)
    const id = searchParams.get("id")
    console.log("[v0] DELETE /api/profesores id:", id)
    ...
    console.error("[v0] DELETE /api/profesores error:", error.message, error.code)
